...
AddrSpace::AddrSpace(OpenFile *executable)
{
    NoffHeader noffH;
    unsigned int i, size;
    int temp_phy_num = -1;      //use bitmap to find a void page
...
// first, set up the translation 
    ...
    pageTable[i].recent = 0;
    pageTable[i].buddy = NULL;
    }
//second, set up a swap area
    swapFile = new char[size];
    for (i = 0; i < size; i++)
      	swapFile[i] = 0; 
...
// then, copy in the code and data segments into memory
    ...
    if (noffH.code.size > 0) {
        ...
        executable->ReadAt(&(swapFile[noffH.code.virtualAddr]),
    ...
    if (noffH.initData.size > 0) {
        ...
        executable->ReadAt(&(swapFile[noffH.initData.virtualAddr]),
